...
from mqtest.kafka import KafkaEventProducer, KafkaTestAdmin, ProducerSettings
from mqtest.sqs import SqsEventClient
from sample_app.local_lab.control import ConsumerControls
from sample_app.local_lab.infrastructure import build_sqs_client, get_queue_url
from sample_app.local_lab.settings import LocalLabSettings
from sample_app.order_api import create_order_app
from sample_app.order_consumer import PostgresOrderStore, StoredOrder
...
def create_local_lab_app() -> FastAPI:
    ...
    def health() -> dict[str, Any]:
        ...
        try:
            ...
            services["postgres"] = {
                "status": "up",
                "detail": f"mqtest · {total} processed orders",
            }
        except Exception as exc:
            services["postgres"] = {
                "status": "down",
...
def _dashboard_html() -> str:
    return (
        resources.files("sample_app.local_lab")
